...
export function mockClockifyApi() {
  ...
  return {
    ...
    mockGetCurrentUser: () => {
      scope.get('/api/v1/user').reply(200, mockApiResponses.user);
      return scope;
    },

    mockGetWorkspaces: () => {
      scope.get('/api/v1/workspaces').reply(200, mockApiResponses.workspaces);
      return scope;
    },
    ...
    mockGetWorkspace: (workspaceId: string) => {
      scope
        ...
        .reply(
          200,
          mockApiResponses.workspaces.find(w => w.id === workspaceId) ||
            mockApiResponses.workspaces[0]
        );
      return scope;
    },
    ...
    mockGetProject: (workspaceId: string, projectId: string) => {
      scope
        ...
        .reply(
          200,
          mockApiResponses.projects.find(p => p.id === projectId) || mockApiResponses.projects[0]
        );
      return scope;
    },
    ...
    mockUpdateTimeEntry: (workspaceId: string, entryId: string) => {
      scope
        .put(`/api/v1/workspaces/${workspaceId}/time-entries/${entryId}`)
        .matchHeader('X-Api-Key', /.+/)
    ...
    mockGetTimeEntry: (workspaceId: string, entryId: string) => {
      scope
        .get(`/api/v1/workspaces/${workspaceId}/time-entries/${entryId}`)
        .matchHeader('X-Api-Key', /.+/)
        .reply(200, mockApiResponses.timeEntries[0]);
      return scope;
    },
    ...
    mockStopTimer: (workspaceId: string, userId: string) => {
      scope
        ...
        .matchHeader('X-Api-Key', /.+/)
        .reply(200, {
          ...mockApiResponses.timeEntries[0],
          timeInterval: {
            ...mockApiResponses.timeEntries[0].timeInterval,
    ...
    mockBulkEditTimeEntries: (workspaceId: string) => {
      scope
        .patch(`/api/v1/workspaces/${workspaceId}/time-entries/bulk`)
        .matchHeader('X-Api-Key', /.+/)
    ...
    mockBulkDeleteTimeEntries: (workspaceId: string) => {
      scope
        .delete(`/api/v1/workspaces/${workspaceId}/time-entries`)
        .matchHeader('X-Api-Key', /.+/)
        .reply(204);
      return scope;
    },
    ...
    mockApiError: (status: number, message: string) => {
      scope.get(/.*/).reply(status, { message });
      return scope;
    },

    mockUnauthorized: () => {
      scope.get(/.*/).reply(401, { message: 'Unauthorized' });
      return scope;
    },

    mockRateLimit: () => {
      scope.get(/.*/).reply(429, { message: 'Rate limit exceeded' });
      return scope;
    },

    mockNotFound: () => {
      scope.get(/.*/).reply(404, { message: 'Not found' });
      return scope;
    },
  };
}
