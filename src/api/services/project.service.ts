...
export class ProjectService {
  ...
  async getAllProjects(workspaceId: string, options?: {
    archived?: boolean;
    name?: string;
    strictName?: boolean;
    clients?: string[];
  ...
  async getProjectById(workspaceId: string, projectId: string, options?: {
    hydrated?: boolean;
  }): Promise<ClockifyProject> {
    return this.client.get<ClockifyProject>(`/workspaces/${workspaceId}/projects/${projectId}`, options);
  }
  ...
  async updateProject(workspaceId: string, projectId: string, data: {
    ...
    return this.client.put<ClockifyProject>(`/workspaces/${workspaceId}/projects/${projectId}`, data);
  }
  ...
  async addUserToProject(workspaceId: string, projectId: string, userId: string, data?: {
    hourlyRate?: {
      amount: number;
      currency: string;
    };
  ...
  async removeUserFromProject(workspaceId: string, projectId: string, userId: string): Promise<void> {
    return this.client.delete(`/workspaces/${workspaceId}/projects/${projectId}/users/${userId}`);
  }

  async updateProjectEstimate(workspaceId: string, projectId: string, estimate: {
    estimate: string;
    type: "AUTO" | "MANUAL";
  }): Promise<ClockifyProject> {
    return this.client.patch<ClockifyProject>(`/workspaces/${workspaceId}/projects/${projectId}/estimate`, estimate);
  }

  async updateProjectMemberships(workspaceId: string, projectId: string, memberships: Array<{
    userId: string;
    hourlyRate?: {
      amount: number;
      currency: string;
    ...
    return this.client.patch<ClockifyProject>(`/workspaces/${workspaceId}/projects/${projectId}/memberships`, {
      memberships
    });
  }
}