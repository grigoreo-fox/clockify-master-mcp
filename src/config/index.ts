...
export const ConfigSchema = z.object({
  ...
  restrictions: z.object({
    ...
    allowedProjects: z.array(z.string()).optional().describe("List of allowed project IDs. If set, only these projects can be accessed/modified"),
    deniedProjects: z.array(z.string()).optional().describe("List of denied project IDs. These projects cannot be accessed/modified"),
    defaultProjectId: z.string().optional().describe("Default project ID to use when not specified"),
    
    // Workspace restrictions
    allowedWorkspaces: z.array(z.string()).optional().describe("List of allowed workspace IDs"),
    defaultWorkspaceId: z.string().optional().describe("Default workspace ID to use when not specified"),
    ...
    allowProjectManagement: z.boolean().default(true).describe("Allow creating/updating/deleting projects"),
    ...
    // Time restrictions
    maxTimeEntryDuration: z.number().optional().describe("Maximum duration for a single time entry in hours"),
    allowFutureTimeEntries: z.boolean().default(false).describe("Allow creating time entries in the future"),
    allowPastTimeEntriesInDays: z.number().default(30).describe("How many days in the past time entries can be created/edited"),
  }).default({}),
  ...
  toolFiltering: z.object({
    enabledCategories: z.array(z.enum([
      ...
      "tag", "task", "report", "bulk", "search"
    ])).default(["user", "workspace", "project", "timeEntry", "report"]).describe("Categories of tools to enable"),
    enabledTools: z.array(z.string()).optional().describe("Specific tools to enable (overrides categories)"),
